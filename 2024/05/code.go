			...
			for _, rule := range internal_relevant_rules {
				...
				if rule[1] == test_number {
					for remainingNumber := range case_line[test_index:] {
						if searchRules(remainingNumber, test_number, rules) && searchRulesBackwards(remainingNumber, test_number, rules) {
...
// 	for i := 0; i < n-1; i++ {
// 		for j := 0; j < n-1; j++ {
// 			if searchRules(case_line[i], case_line[j], rules) && searchRulesBackwards(case_line[i], case_line[j], rules) {
// 				a := case_line[i]
// 				b := case_line[j]
// 				case_line[j] = a
// 				case_line[i] = b
...
func isBAfterA(a int, b int, rules [][]int) bool {
	for _, rule := range rules {
...
func searchRulesBackwards(a int, b int, rules [][]int) bool {
	if a == b {
		return true
	}
	found := false
	for _, rule := range rules {
		if rule[1] == b {
			found = searchRules(a, rule[0], rules)
		}
	}
	return found
}
